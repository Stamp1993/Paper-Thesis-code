...
public:
	struct pole_state : public state {
		...
		pole_state(bool f) : state(18, true), pos(0), vel(0), angle1(0), angVel1(0), angAcc1(0), angle2(0), angVel2(0), angAcc2(0) {
			angle1 += noise();//pole should start falling
			angle2 += noise();
		...
		pole_state(const pole_state &oth) : state(18), pos(oth.pos), vel(oth.vel), acc(oth.acc), angle1(oth.angle1), angVel1(oth.angVel1), angAcc1(oth.angAcc1), angle2(oth.angle2), angVel2(oth.angVel2), angAcc2(oth.angAcc2) {
			//copy constructor
			//	cout << "copy" << endl;
	...
	pole_state t_step(state *in, punchDir dir) {
		...
		double angle2 = currentState.info[12]+currentState.info[13];
		double angVel2 = currentState.info[14]+currentState.info[15];
		double angAcc2 = currentState.info[16]+currentState.info[17];
